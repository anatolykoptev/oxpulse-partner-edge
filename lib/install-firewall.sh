#!/usr/bin/env bash
# lib/install-firewall.sh — FROZEN back-compat duplicate of lib/firewall-lib.sh
# (Phase 6 naming reclassification, strangler-harden series).
#
# This file was renamed to lib/firewall-lib.sh (shared: sourced by both
# install.sh and lib/reconcile.sh's converge loop, hence the *-lib.sh
# classification — install-*.sh names are reserved for install.sh-only
# modules). install.sh's `_install_lib_source install-firewall.sh` and
# upgrade.sh's `_stage_lib "install-firewall.sh" ...` / tier-3 REPO_RAW
# fetch still reference this OLD literal filename this phase (out of this
# task's edit scope) — including the raw.githubusercontent.com git-tree
# fetch tier, which serves a git SYMLINK's blob as literal target-path text
# rather than dereferenced content. A real git symlink here would therefore
# die-on-fetch on every fresh curl|bash install; this is a full content
# duplicate instead, kept byte-for-byte in sync with lib/firewall-lib.sh.
#
# DO NOT edit this file's logic independently of lib/firewall-lib.sh.
# Kill criteria: delete this file (+ its lib/lib-checksums.txt entry) one
# release after install.sh/upgrade.sh are updated to reference
# lib/firewall-lib.sh directly and all pinned partner edges have upgraded
# past the Phase 6 rename tag. CI grep-guard (.github/workflows/release.yml)
# fails the build on a stray NEW reference to this old name in lib/*.sh.
#
