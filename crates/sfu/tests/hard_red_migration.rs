...
// ─── Test 9 ───────────────────────────────────────────────────────────────────
//
// Task #18: pacer min-tick floor + softened suspend-debounce.
//
// `Registry::update_pacer_layers` fires once per `MediaData` (~20-30ms nominal
// cadence) with no minimum-tick spacing. The kit's `SUSPEND_STREAK` debounce
// (2 consecutive below-threshold ticks) assumes ticks land roughly
// `PACER_MIN_TICK_INTERVAL` (100ms) apart; without a floor, two below-threshold
// ticks inside the same ~20-40ms burst satisfy the debounce and force a
// spurious `SuspendVideo`, dropping video for a transient loss blip.
//
// Behind `SFU_PACER_FLOOR=1`: (1) `update_pacer_layers` gates the pacer FSM
// advance to at most once per `PACER_MIN_TICK_INTERVAL` per subscriber, so a
// burst inside that window only counts as one real advance; (2)
// `oxpulse_partner_edge_pacer_config` raises `suspend_streak` from the kit
// default (2, ~200ms) to `SOFTENED_SUSPEND_STREAK` (3, ~300ms) — see
// `crate::pacer_floor` docs for the competitor-research grounding.
//
// This test asserts BOTH halves so a regression in either direction is caught:
// the legacy/default-off cadence still reproduces the bug (proves the test
// harness actually exercises the bug, not just the fix), and the floored
// cadence resists the same burst but still suspends on sustained low BWE.
#[test]
