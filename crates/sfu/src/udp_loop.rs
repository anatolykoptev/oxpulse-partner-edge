...
mod tests {
    ...
    async fn flush_transmits_preserves_order_and_skips_bogon() {
        ...
        // `is_bogon` (correctly) rejects loopback, RFC1918, and link-local as
        // send destinations — real ICE peers never live there — so the only
        // address a local receiver can bind that still passes the bogon filter
        // is a global-scope one. This host's IPv4 is RFC1918 behind NAT, but it
        // has a real global-scope IPv6 address, and sending UDP to your own
        // globally-routable address is delivered locally by the kernel (verified
        // empirically on this box). Discover it with the connect-without-sending
        // trick: `connect()` on a UDP socket is a pure routing-table lookup — no
        // packet leaves the host.
        //
        // Hosts with NO global IPv6 route (e.g. GitHub-hosted CI runners, which
        // have no IPv6 connectivity) fail this lookup. Rather than panic, we then
        // run the portable subset — drive the REAL `flush_transmits` with a
        // bogon-destined transmit and assert it is dropped before `send_to` (the
        // security-critical property: a bogon packet must never leave the process)
        // plus the Vec-drain — and skip only the two-same-dest network-receipt
        // order check, which needs a real global-scope receiver.
        let local_v6: Option<std::net::IpAddr> = std::net::UdpSocket::bind("[::]:0")
            .and_then(|probe| {
                probe.connect("[2001:4860:4860::8888]:80")?;
                probe.local_addr()
            })
            .ok()
            .map(|addr| addr.ip());
        ...
        let Some(local_v6) = local_v6 else {
            eprintln!(
                "flush_transmits_preserves_order_and_skips_bogon: no global IPv6 route on this \
                 host — running the bogon-drop + Vec-drain subset and skipping the \
                 network-receipt order check (which needs a global-scope receiver)"
            );
            // The send socket is never used to send here — the single transmit is
            // bogon-destined and dropped before `send_to` — but `flush_transmits`
            // still requires a socket argument. Loopback binds on every host.
            let send_sock = UdpSocket::bind("127.0.0.1:0")
                .await
                .expect("bind loopback send socket");
